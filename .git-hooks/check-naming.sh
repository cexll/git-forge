...
# Wired into pre-commit via .git-hooks/pre-commit.
#
# See AGENTS.md § Code Canonicality. Runs on bare macOS bash 3.2.
...
What to do:
  - Refactoring: rename the new file over the old one; let git track history.
  - Separate concept: rename to something descriptive (no suffix).
  - Throwaway: keep it outside the repo (e.g., /tmp).
