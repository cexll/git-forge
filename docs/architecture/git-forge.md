...
- Event `actor` (see below): the invoking repo's `user.email`. When `user.email` is absent or empty — not set, or set to an empty/whitespace value, in repo, global, or system config — commands still succeed and the actor falls back to `forge@localhost` (the same default the store commit signature uses); a repo without an email is never blocked from writing events. The identity config-failure policy is stage-specific (F-028): a failure to **open** the repo's config at all (`repo.config()`; an environment fault) propagates as a clean error — no silent fallback, no empty actor, no event written (under the pinned libgit2 1.9.6 this is an unexercised defensive path, documented by probe record — see Event Identity); a failure to **look up** `user.email` (`get_string`; absent key or a value that cannot be read as a usable string) falls back to `forge@localhost`, with commands still succeeding. `user.name` does NOT gate the actor: the actor is always the configured email.
...
- **No-email fallback**: when `user.email` is not configured or is empty (absent, or set to an empty/whitespace value, in repo, global, and system config), the actor falls back to `forge@localhost` and commands still succeed; the store commit signature uses the same default (`git-forge` / `forge@localhost`). `user.name` does not control the actor.
- **Config-failure policy (two-stage, F-028)**: identity config failures are stage-specific. STAGE A — opening the repo's own config (`repo.config()`, the early `?` in `actor()`) is a repo-access environment fault: it **propagates** as a clean error (actor resolution is impossible, and silent fallback or an empty actor would mask a broken repo); no event is written and no forge ref mutates. On the pinned libgit2 1.9.6, probe fixtures produced no clean `repo.config()` error on an already-open repository, so STAGE A is an unexercised defensive error path documented by source inspection + a probe record (`.specs/git-forge-contract-fix/evidence/assertions-contract-fix/VAL-117-STAGE-A-probe.txt`), not pinned by a regression; no snapshot-loading invariant is claimed. STAGE B — looking up `user.email` (`get_string`) **falls back** to `forge@localhost`: an absent key (NotFound), an empty/whitespace value, or any value that cannot be read as a usable string (a malformed/unreadable value) all mean "no usable identity", so commands still succeed, matching the contract sentence that a repo without a usable identity is never blocked from writing events.
